...
| 10 | Flutter release build setup (Fastlane) | Pending | P3 — **Deferred until closer to release.** Configure Fastlane for iOS (TestFlight) and Android (APK/Play Store Internal Testing). Tasks: generate signing keys (iOS provisioning profile + Android keystore), create `Fastfile` with `beta` and `release` lanes, configure `Appfile` and `Matchfile`, add build number auto-increment, set up environment-specific build flavors (dev/staging/prod), add `fastlane` CI lane to GitHub Actions, configure code obfuscation (`--obfuscate --split-debug-info`). Target users: Mom (iOS via TestFlight), Brother (Android via direct APK or Play Internal Testing). |
