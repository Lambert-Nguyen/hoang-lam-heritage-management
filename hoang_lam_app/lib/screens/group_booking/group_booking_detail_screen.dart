...
class GroupBookingDetailScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<GroupBookingDetailScreen> createState() =>
      _GroupBookingDetailScreenState();
}

class _GroupBookingDetailScreenState
    extends ConsumerState<GroupBookingDetailScreen> {
  ...
  Widget _buildBottomBar(GroupBooking booking) {
    if (booking.status == GroupBookingStatus.checkedOut ||
          ...
          children: [
            if (booking.status == GroupBookingStatus.tentative) ...[
              ...
              Expanded(
                child: AppButton(
                  label: l10n.confirm,
                  onPressed: _isLoading ? null : () => _confirmBooking(booking),
  ...
  void _handleMenuAction(String action, GroupBooking booking) {
    switch (action) {
      case 'edit':
        context.push('/group-bookings/${booking.id}/edit');
      case 'confirm':
  ...
  Future<void> _confirmBooking(GroupBooking booking) async {
    final confirmed = await _showConfirmDialog(
      ...
      context.l10n.confirmGroupBooking,
    );
    if (confirmed != true) return;
    setState(() => _isLoading = true);
    try {
      final result = await ref
          .read(groupBookingNotifierProvider.notifier)
  ...
  Future<void> _checkIn(GroupBooking booking) async {
    if (booking.roomNumbers.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(context.l10n.assignRoomsFirstMsg),
  ...
  Future<void> _cancelBooking(GroupBooking booking) async {
    final reason = await _showInputDialog(
      ...
      context.l10n.enterCancelReason,
    );
  ...
  Future<void> _showRoomAssignmentDialog(GroupBooking booking) async {
    ...
    try {
      final success = await ref
          .read(groupBookingNotifierProvider.notifier)
          .assignRooms(booking.id, roomIds);
      if (success != null && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(context.l10n.roomsAssignedSuccess)),
        );
        ref.invalidate(groupBookingByIdProvider(widget.bookingId));
      }
    } finally {
